...
export class Logger {
	...
	constructor(name: string = "Logger") {
		...
		this.currentLevel = (process.env.LOG_LEVEL as LogLevel) || "debug";
		...
		try {
			...
			if (remainingFiles.length > this.maxLogFiles) {
				...
				filesToDelete.forEach(file => {
					...
					console.log(`已刪除舊日誌檔案: ${file.name}`);
				});
			}
		} catch (error) {
	...
	success(message: string, data: any = null): void {
		if (this.logLevels[this.currentLevel] >= this.logLevels.info) {
			...
			console.log(`\x1b[32m${formattedMessage}\x1b[0m`); // 綠色
	...
	setLevel(level: LogLevel): void {
		if (this.logLevels.hasOwnProperty(level)) {
			...
			this.info(`日誌等級已設定為: ${level}`);
		} else {
			this.error(`無效的日誌等級: ${level}`);
		}
	}
	...
	getLogStats(): LogStats | { error: string } {
		try {
			...
			return {
				...
				maxSize: this.formatFileSize(this.maxLogSize)
			};
		} catch (error) {
